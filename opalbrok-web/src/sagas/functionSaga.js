...
export function* invoiceCalculate(action) {
    try {
        ...
        yield put({type: types.INVOICE_CALCULATE_SUCCESS, data: data.data});
...
export function* addProducts(action) {
    try {
        ...
        yield put({type: types.CATALOG_INSERT_SUCCESS, data: data.data});
...
export function* generateResults(action) {
    try {
        ...
        yield put({type: types.INVOICE_GENERATE_RESULT_SUCCESS, data: data.data});
